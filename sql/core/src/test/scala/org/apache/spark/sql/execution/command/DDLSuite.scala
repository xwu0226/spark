...
abstract class DDLSuite extends QueryTest with SQLTestUtils {
  ...
  alterTypes.foreach { alterType =>
    supportedNativeFileFormatsForAlterTableAddColumns.foreach { provider =>
      test(s"alter datasource table $alterType columns - $provider") {
        withTable("t1") {
  ...
  alterTypes.foreach { alterType =>
    supportedNativeFileFormatsForAlterTableAddColumns.foreach { provider =>
      test(s"alter datasource table $alterType columns - partitioned - $provider") {
        withTable("t1") {
          ...
          if (alterType == "ADD") {
            ...
            sql("ALTER TABLE t1 REPLACE COLUMNS (c1 int, c3 int)")
          }
          checkAnswer(
            spark.table("t1"),
            Seq(Row(1, null, 2))
  ...
  supportedNativeFileFormatsForAlterTableAddColumns.foreach { provider =>
    test(s"alter datasource table replace to drop columns - partitioned $provider") {
      withTable("t1") {
        ...
        sql("INSERT INTO t1 PARTITION(c2 = 2) VALUES (1, 3)")
  ...
  supportedNativeFileFormatsForAlterTableAddColumns.foreach { provider =>
    test(s"alter datasource table replace to change order - partitioned $provider") {
      withTable("t1") {
        ...
        sql("INSERT INTO t1 partition(c3=1) VALUES (1, 3)")

        sql("ALTER TABLE t1 REPLACE COLUMNS (c2 int, c1 int)")
